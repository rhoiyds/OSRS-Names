...
public class TradeResource {
     ...
     */
    @GetMapping("/trades")
    public List<Trade> getAllTrades(@RequestParam(required = false) String filter) {
        if ("offer-is-null".equals(filter)) {
            log.debug("REST request to get all Trades where offer is null");
            return tradeService.findAllWhereOfferIsNull();
        }
        if ("middlemanrequest-is-null".equals(filter)) {
            log.debug("REST request to get all Trades where middlemanRequest is null");
            return tradeService.findAllWhereMiddlemanRequestIsNull();
        }
        if ("rating-is-null".equals(filter)) {
            log.debug("REST request to get all Trades where rating is null");
            return tradeService.findAllWhereRatingIsNull();
        }
