...
export class PaymentService {
  ...
  create(payment: IPayment): Observable<EntityResponseType> {
    return this.http
      .post<IPayment>(this.resourceUrl, payment, { observe: 'response' })
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      timestamp: payment.timestamp != null && payment.timestamp.isValid() ? payment.timestamp.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.timestamp = payment.timestamp != null ? moment(payment.timestamp) : null;
      });
    }
    return res;
  }
